...
export class SpineDebugRenderer implements ISpineDebugRenderer {
  private registeredSpines: Map<
    SpineBase<ISkeleton, ISkeletonData, IAnimationState, IAnimationStateData>,
    DebugDisplayObjects
  > = new Map();
  ...
  private drawClippingFunc(
    ...
    for (let i = 0, len = slots.length; i < len; i++) {
      ...
      const clippingAttachment: IClippingAttachment =
  ...
  private drawPathsFunc(
    ...
    for (let i = 0, len = slots.length; i < len; i++) {
      ...
      const pathAttachment = attachment as IVertexAttachment & {
        closed: boolean;
      };
