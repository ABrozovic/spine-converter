import { ALPHA_MODES, MIPMAP_MODES, SCALE_MODES } from "@pixi/constants";
import { Texture, type BaseTexture } from "@pixi/core";
...
export class TextureAtlas implements Disposable {
  ...
  constructor(
    ...
    textureLoader?: (
      path: string,
      loaderFunction: (tex: BaseTexture) => any
    ) => any,
    ...
    let iterateParser = () => {
      while (true) {
        ...
        if (line.trim().length == 0) {
          ...
          let orig = new Rectangle(
            0,
            0,
            region.originalWidth,
            region.originalHeight
          );
          let trim = new Rectangle(
            region.offsetX,
