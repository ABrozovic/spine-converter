...
export class TranslateTimeline extends CurveTimeline {
  ...
  apply(
    ...
    if (time >= frames[frames.length - TranslateTimeline.ENTRIES]) {
      ...
      // Interpolate between the previous frame and the current frame.
      const frame = Animation.binarySearch(
        frames,
        time,
...
export class DeformTimeline extends CurveTimeline {
  ...
  getPropertyId() {
    return (
      (TimelineType.deform << 27) + Number(this.attachment.id) + this.slotIndex
    );
  }
    ...
    if (time < frames[0]) {
      ...
      switch (blend) {
        ...
        case MixBlend.first:
          ...
          const deform: Array<number> = Utils.setArraySize(
            deformArray,
            vertexCount
          );
  ...
  apply(
    ...
    constraint.mix +=
      (mix +
        ...
        constraint.mix) *
      alpha;
    constraint.bendDirection = Math.floor(
