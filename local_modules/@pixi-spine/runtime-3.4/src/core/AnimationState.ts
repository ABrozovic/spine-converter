...
export class AnimationState implements IAnimationState<AnimationStateData> {
  ...
  setEmptyAnimation(trackIndex: number, mixDuration: number) {
    const entry = this.setAnimationWith(
      trackIndex,
  ...
  addEmptyAnimation(trackIndex: number, mixDuration: number, delay: number) {
    ...
    const entry = this.addAnimationWith(
      trackIndex,
      AnimationState.emptyAnimation,
      false,
      delay
  ...
  computeHold(entry: TrackEntry) {
    ...
    const timelineDipMix = Utils.setArraySize(
    ...
    for (let i = 0; i < objects.length; i += 2) {
      ...
      switch (type) {
        ...
        case EventType.event:
          ...
          entry.onEvent &&
            this.deprecateStuff() &&
            entry.onEvent(entry.trackIndex, event);
          this.animState.onEvent &&
            this.deprecateStuff() &&
            this.animState.onEvent(entry.trackIndex, event);
          break;
      }
    }
    this.clear();
