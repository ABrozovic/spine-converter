      ...
      for (let i = 0; i < root.skins.length; i++) {
        ...
        if (skinMap.transform) {
          for (let ii = 0; ii < skinMap.transform.length; ii++) {
            const constraint = skeletonData.findTransformConstraint(
        ...
        if (skinMap.path) {
          for (let ii = 0; ii < skinMap.path.length; ii++) {
            const constraint = skeletonData.findPathConstraint(
              skinMap.path[ii]
            );
        ...
        for (const timelineName in boneMap) {
          ...
          if (timelineName === "rotate") {
            ...
            duration = Math.max(
              duration,
              timeline.frames[
                (timeline.getFrameCount() - 1) * TranslateTimeline.ENTRIES
              ]
            );
          } else
            throw new Error(
    ...
    if (map.transform) {
      for (const constraintName in map.transform) {
        ...
        duration = Math.max(
          duration,
          timeline.frames[
            (timeline.getFrameCount() - 1) * TransformConstraintTimeline.ENTRIES
          ]
        );
      }
    }
        ...
        for (const slotName in deformMap) {
          ...
          for (const timelineName in slotMap) {
            ...
            for (let j = 0; j < timelineMap.length; j++) {
              ...
              const verticesValue: Array<Number> = this.getValue(
                valueMap,
                "vertices",
                null
              );
